...
from objects.image_writer import write_image_from_tree
from objects.geometry_writer import write_geometry_from_tree
...
if __name__ == "__main__":
    ...
    for i in range(0, 5):
        ...
        tree = Tree.generate_tree({'branch_length': 0.08,
                                   'turn_factor': 0.1,
                                   'leaf_start': 0.1,
                                   'number_of_leaves': 200})
        write_image_from_tree("lengthy_{}.png".format(i), tree)

        # Pick one of the iterations, write it as a object
        if i == 3:
            write_geometry_from_tree("lengthy_tree.obj", tree, leaves=False)

    # Generate some wiry trees. These short branches turn fast, and grow after each leaf
    for i in range(0, 5):
        print("Generating wiry tree {}".format(i))

        tree = Tree.generate_tree({'min_distance': 0.02,
                                   'max_distance': 0.40,
                                   'branch_length': 0.04,
                                   'turn_factor': 0.60,
                                   'leaf_start': 0.1,
                                   'number_of_leaves': 400})
        write_image_from_tree("wiry_{}.png".format(i), tree)

        if i == 0:
            write_geometry_from_tree("wiry_tree.obj", tree, leaves=False)
    ...
    for i in range(0, 5):
        ...
        tree = Tree.generate_tree({'min_distance': 0.02,
                                   ...
                                   'leaf_start': 0.85,
                                   'number_of_leaves': 50})
        write_image_from_tree("weird_{}.png".format(i), tree)

    # Generate a 2d tree
    tree = Tree.generate_tree({'min_distance': 0.048,
                               'max_distance': 0.4,
                               'branch_length': 0.03,
                               'turn_factor': 0.35,
                               'leaf_start': 0.30,
                               ...
                               'limit_2d': True})
    write_image_from_tree("2d_tree.png", tree)
    write_geometry_from_tree("2d_tree.obj", tree, leaves=False)
